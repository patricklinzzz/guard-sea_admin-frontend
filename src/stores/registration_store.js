...
export const useRegistrationStore = defineStore('registration', () => {
  ...
  const fetchRegistrationByEvent = async (eventId) => {
    ...
    try {
      ...
      const res = await axios.get(
        `http://localhost:8888/guard-sea_api/events/get_registrations.php?activity_id=${eventId}`
      )
  ...
  const fetchRegistrationByUser = async (userId) => {
    ...
    try {
      ...
      const res = await axios.get(
        `http://localhost:8888/guard-sea_api/registrations/get_registrations.php?user_id=${userId}`
      )
      if (res.data.status === 'success') {
        registrationList.value = res.data.data
      } else {
  ...
  const addRegistration = async (registration) => {
    try {
      const res = await axios.post(
        'http://localhost:8888/guard-sea_api/registrations/add_registration.php',
        registration
      )
      if (res.data.status === 'success') {
